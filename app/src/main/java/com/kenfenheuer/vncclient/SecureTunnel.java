...
public class SecureTunnel implements X509TrustManager
{
	...
	{		
        ...
        Message msg = Message.obtain(m_messageBus, 1, remoteCertificate);
	...
	public static boolean isSignatureEqual(String expectedSignature, byte[] remoteData) throws Exception
	{
		String trimExpectedSignature = expectedSignature.trim();
		if (isNullOrEmptry(trimExpectedSignature))
			return false;
		String remoteSig = computeSignatureByAlgorithm(remoteData);
		if (remoteSig.equalsIgnoreCase(trimExpectedSignature))
			return true;
		return false;
	}
	
	public static String computeSignatureByAlgorithm(byte[] data) throws NoSuchAlgorithmException
	{
		MessageDigest digest  =  MessageDigest.getInstance("SHA-1");

		byte[] fingerprint = digest.digest(data);
		String fingerprintHex = toHexString(fingerprint).trim();
