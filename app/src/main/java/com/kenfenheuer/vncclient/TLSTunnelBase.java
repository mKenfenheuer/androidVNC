...
public TLSTunnelBase (Socket sock_) {
  ...
  public void setup (RfbProto cc) throws Exception {
    try {
      ...
      String[] protocols = ((SSLSocket)sslsock).getSupportedProtocols();
      ((SSLSocket)sslsock).setEnabledProtocols(protocols);


      sslsock.setTcpNoDelay(true);
      sslsock.setSoTimeout(2000);
    ...
    {
      throw new Exception("TLS handshake failed " + e.getCause().getMessage());
    }
