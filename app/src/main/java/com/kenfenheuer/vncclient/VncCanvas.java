...
public class VncCanvas extends ImageView {
	...
	private void validateX509Cert (final X509Certificate cert) {
		...
		try {
			...
			String message = "";
			if (certMismatch) {
				message = "WARNING: The saved x509 certificate or passed in signature does not match the current server certificate! This could be a man-in-the-middle attack. If you are aware of the key change you may proceed.\n\n";
			}
			...
			Utils.showYesNoPrompt(getContext(),"Connecting to " +
					connection.getAddress (),
					certInfo,
					signatureYes, signatureNo);
		} catch (NoSuchAlgorithmException e2) {
			e2.printStackTrace();
			Utils.showFatalErrorMessage(getContext(),"Could not get cert signature");
		} catch (CertificateEncodingException e) {
			e.printStackTrace();
			Utils.showFatalErrorMessage(getContext(),"Could not get encoding");
		}
	}
	...
	void connectAndAuthenticate(String us,String pw) throws Exception {
		...
		rfb = new RfbProto(connection.getAddress(), connection.getPort(), this);
		...
		if (secType == RfbProto.SecTypeTight) {
			...
			rfb.authenticateTLS();
			authType = rfb.negotiateSecurity(bitPref);
		} else {
			authType = secType;
		}
